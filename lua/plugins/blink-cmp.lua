...
return {
	{
		...
		dependencies = {
			...
			"saghen/blink.compat", -- Required for external sources
		},
		lazy = false,
		...
		opts = function(_, opts)
			...
			local function has_closing_paren_ts(row, col)
				...
				while node and depth < 50 do
					...
					if t == "call_expression" or t == "function_call" then
						...
						local ln = vim.api.nvim_buf_get_lines(
							0,
							er,
							er + 1,
							false
			...
			return vim.tbl_deep_extend("force", opts or {}, {
				...
				completion = {
					...
					documentation = {
						...
						window = {
							...
							border = "rounded",
							winhighlight =
								"Normal:BlinkCmpDoc,FloatBorder:BlinkCmpDocBorder,CursorLine:BlinkCmpDocCursorLine",
						},
					},
				},
				...
				sources = {
					-- Added 'otter' to the default list items
					default = {
						"snippets",
						"lsp",
						"path",
					...
					providers = {
						-- Configured otter through the blink.compat provider bridge
						otter = {
							name = "otter",
						...
						lsp = {
							...
							transform_items = function(ctx, items)
								...
								for i = 1, count do
									...
									if kind
										and kinds[kind]
										and item.insertTextFormat == format
									then
										local it = item.insertText
...

	{
		...
		opts = {
			...
			buffers = {
				-- otter's synthetic js buffer doesn't need to hit disk
				write_to_disk = false,
			},
		...
		config = function(_, opts)
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function()
					-- languages, completion, diagnostics
